...
def select_signals(
    ...
    *,
    ...
    if kospi_candles and index_blocks_entries(kospi_candles, skip):
    ...
    accepted.sort(key=lambda s: (s.foreign_net + s.institution_net), reverse=True)
    return SignalDecision(accepted=accepted, rejected=rejected)
